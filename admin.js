(() => {
  ...
  function renderLoading(target, profile) {
    target.innerHTML = `<div class="admin-shell glass admin-loading-shell"><div class="admin-head"><div><span class="eyebrow">CONTROL CENTER</span><h1>Admin</h1><p>Loading the latest Flash Games control data…</p></div><span class="admin-role">${esc(profile.role || 'admin')}</span></div><div class="admin-panel admin-loading-card">${icon('loader-circle')}<div><strong>Connecting to Flash Games data</strong><small>Loading maintenance status, catalogue and users.</small></div></div></div>`;
    refreshIcons(target);
  }
  ...
  function bind(target, user, profile, toast) {
    ...
    $('#adminMaintenanceOn', target)?.addEventListener('click', async () => {
      try {
        await saveMaintenance(user, true, message());
        toast('Update lock enabled', 'Normal users are now blocked from the app.', 'success');
        await render(target, user, profile, toast);
      } catch (error) {
        toast('Could not enable maintenance', error.message || 'Firebase rejected the change.', 'error');
      }
    });
    $('#adminMaintenanceOff', target)?.addEventListener('click', async () => {
      try {
        await saveMaintenance(user, false, message());
  ...
  async function render(target, user, profile, toast) {
    ...
    target.innerHTML = `<div class="admin-shell glass"><div class="admin-head"><div><span class="eyebrow">CONTROL CENTER</span><h1>Admin</h1><p>Manage Flash Games without leaving the app.</p></div><span class="admin-role">${esc(profile.role || 'admin')}</span></div><section class="admin-panel"><span class="eyebrow">OVERVIEW</span><h2>Dashboard</h2><p>The live Flash Games data layer currently exposes <strong>${catalogue.games.length.toLocaleString()}</strong> real games. Current release: <strong>${esc(updates.version)}</strong>. Users loaded: <strong>${users.length}</strong>.</p></section><section class="admin-panel"><span class="eyebrow">RELEASE CONTROL</span><h2>Maintenance mode</h2><p>Enable this before publishing an update. Normal users are blocked from the app while authorized admins can still use the control center.</p><div class="maintenance-card ${maintenance.enabled ? 'locked' : ''}"><div><strong>${maintenance.enabled ? 'Update lock is ON' : 'Site is public'}</strong><small>${esc(maintenance.message || 'No maintenance message set.')}</small></div><span class="switch ${maintenance.enabled ? 'on' : ''}"><i></i></span></div><textarea id="adminMaintenanceMessage" class="admin-input" rows="3" placeholder="Maintenance message">${esc(maintenance.message || 'Flash Games is being updated. Please check back soon.')}</textarea><div class="admin-actions"><button class="btn primary" id="adminMaintenanceOn">${icon('lock')} Lock for update</button><button class="btn" id="adminMaintenanceOff">${icon('lock-open')} Re-open site</button></div></section><section class="admin-panel"><span class="eyebrow">CATALOGUE</span><h2>Games</h2><p>The Store discovers the real Offline HTML Games Pack instead of maintaining a tiny hard-coded list.</p><div class="admin-actions"><button class="btn" id="refreshCatalogue">${icon('refresh-cw')} Refresh ${catalogue.games.length.toLocaleString()} games</button><button class="btn" id="clearGameCacheAdmin">${icon('trash-2')} Clear installed cache</button></div></section><section class="admin-panel"><span class="eyebrow">USER MANAGEMENT</span><h2>Users</h2><p>Firebase users are read from the shared Mobify users collection.</p><div class="admin-user-list">${users.map((item) => `<div class="setting-action"><span><span class="avatar"><img src="${esc(item.photoURL || item.photo || item.pfp || './offline/logo.png')}" alt=""></span><span><strong>${esc(item.username || item.displayName || item.email || item.id)}</strong><small>${esc(item.email || item.role || 'User')}</small></span></span><span class="admin-role">${esc(item.role || 'user')}</span></div>`).join('') || '<p>No user documents could be loaded.</p>'}</div></section><section class="admin-panel"><span class="eyebrow">UPDATES</span><h2>Release history</h2><p>Public changelog data comes from <code>update.json</code>. The release view supports the green plus, yellow hammer, and red X markers.</p><button class="btn" data-route="updates">${icon('history')} Open changelog</button></section></div>`;
    bind(target, user, profile, toast);
    return true;
  }
