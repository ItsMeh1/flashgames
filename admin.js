(() => {
  ...
  const PAGE_SIZE = 12;
  const state = { users: [], page: 1, selected: null, usersLoaded: false, loadingUsers: false };
  ...
  async function getProfile(user) {
    ...
    if (!db || !user) return {};
    try { const snap = await db.collection('users').doc(user.uid).get(); return snap.exists ? snap.data() : {}; } catch { return {}; }
  }

  async function readMaintenance() {
    const db = window.__flashFirebase?.db;
    if (!db) return { enabled: false, message: '' };
    try { const snap = await db.collection('settings').doc('maintenance').get(); return snap.exists ? snap.data() : { enabled: false, message: '' }; } catch { return { enabled: false, message: '' }; }
  }
  ...
  async function loadUsers() {
    if (state.usersLoaded) return state.users;
    if (state.loadingUsers) return state.users;
    const db = window.__flashFirebase?.db;
    if (!db) return [];
    state.loadingUsers = true;
    try {
      const snapshot = await db.collection('users').limit(500).get();
      state.users = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })).sort((a, b) => String(a.username || a.displayName || a.email || a.id).localeCompare(String(b.username || b.displayName || b.email || b.id), undefined, { sensitivity: 'base', numeric: true }));
  ...
  function refreshIcons(root) { if (window.lucide?.createIcons) requestAnimationFrame(() => window.lucide.createIcons({ root, attrs: { 'stroke-width': 1.5 } })); }
  ...
  function openUserManagement(users, user, toast) {
    ...
    if (!backdrop) return;
    state.users = users; state.page = 1; state.selected = null; backdrop.hidden = false; renderUserManagement(user, toast);
  }

  function renderUserManagement(user, toast) {
    const backdrop = $('#userManagementBackdrop'), list = $('#adminMembersList'), pager = $('#adminMembersPager'), count = $('#adminMembersCount');
    if (!backdrop || !list || !pager || !count) return;
    const pages = Math.max(1, Math.ceil(state.users.length / PAGE_SIZE)); state.page = Math.min(state.page, pages);
    ...
    $$('.admin-member-row', list).forEach((row) => row.addEventListener('click', () => { state.selected = state.users.find((item) => item.id === row.dataset.memberId) || null; window.__flashModerationSelectedId = state.selected?.id || null; renderUserEditor(user, toast); }));
  ...
  function renderUserEditor(user, toast) {
    ...
    const close = () => { state.selected = null; window.__flashModerationSelectedId = null; editor.hidden = true; $('#adminMembersList')?.removeAttribute('hidden'); $('#adminMembersPager')?.removeAttribute('hidden'); };
    $('#closeAdminUserEditor')?.addEventListener('click', close); $('#cancelAdminUserEdit')?.addEventListener('click', close);
    $('#saveAdminUserEdit')?.addEventListener('click', async () => {
      try {
        const changes = { username: $('#editUserName').value.trim(), displayName: $('#editUserName').value.trim(), role: $('#editUserRole').value, pfp: $('#editUserPhoto').value.trim(), disabled: $('#editUserDisabled').checked };
        await saveUser(user, item.id, changes); Object.assign(item, changes); toast('Member updated', `${changes.username || item.id} was updated.`, 'success'); close(); renderUserManagement(user, toast);
      } catch (error) { toast('Could not update member', error.message || 'Firebase rejected the change.', 'error'); }
    });
    $('#adminMembersList')?.setAttribute('hidden', ''); $('#adminMembersPager')?.setAttribute('hidden', ''); editor.hidden = false; refreshIcons(editor);
  }

  async function openMembers(user, toast) {
    if (state.loadingUsers) return;
    const button = $('#openMemberManagement');
    if (button) { button.disabled = true; button.classList.add('is-loading'); }
    try {
      const users = await loadUsers();
      openUserManagement(users, user, toast);
    } catch (error) {
  ...
  function bind(target, user, profile, toast) {
    ...
    $('#openMemberManagement', target)?.addEventListener('click', () => openMembers(user, toast));
    refreshIcons(target);
  }
  ...
  async function render(target, user, profile, toast) {
    ...
    target.innerHTML = `<div class="admin-shell glass"><div class="admin-head"><div><span class="eyebrow">CONTROL CENTER</span><h1>Admin</h1><p>Manage Flash Games without leaving the app.</p></div><span class="admin-role">${esc(profile.role || 'admin')}</span></div><div class="admin-compact-grid"><section class="admin-panel compact"><span class="eyebrow">OVERVIEW</span><h2>Dashboard</h2><div class="admin-metrics"><div><strong>${catalogue.games.length.toLocaleString()}</strong><small>Games cached</small></div><div><strong>—</strong><small>Members</small></div><div><strong>${esc(updates.version)}</strong><small>Release</small></div></div></section><section class="admin-panel compact"><span class="eyebrow">RELEASE CONTROL</span><h2>${maintenance.enabled ? 'Update lock active' : 'Site is public'}</h2><p>${esc(maintenance.message || 'Use maintenance mode before publishing a release.')}</p><div class="maintenance-card"><div><strong>${maintenance.enabled ? 'Locked for updates' : 'Public access'}</strong><small>Admins can always access this workspace.</small></div><span class="switch ${maintenance.enabled ? 'on' : ''}"><i></i></span></div><textarea id="adminMaintenanceMessage" class="admin-input compact-input" rows="2" placeholder="Maintenance message">${esc(maintenance.message || 'Flash Games is being updated. Please check back soon.')}</textarea><div class="admin-actions"><button class="btn primary" id="adminMaintenanceOn">${icon('lock')} Lock</button><button class="btn" id="adminMaintenanceOff">${icon('lock-open')} Open</button></div></section></div><section class="admin-panel compact"><div class="admin-section-head"><div><span class="eyebrow">CATALOGUE</span><h2>Games</h2><p>${catalogue.games.length.toLocaleString()} cached games. Refresh only when you want to contact GitHub.</p></div><div class="admin-actions"><button class="btn" id="refreshCatalogue">${icon('refresh-cw')} Refresh</button><button class="btn" id="clearGameCacheAdmin">${icon('trash-2')} Clear cache</button></div></div></section><section class="admin-panel compact"><div class="admin-section-head"><div><span class="eyebrow">MEMBERS</span><h2>User management</h2><p>Members are loaded only when you open the manager.</p></div><button class="btn primary" id="openMemberManagement">${icon('users-round')} View members</button></div></section><section class="admin-panel compact"><div class="admin-section-head"><div><span class="eyebrow">UPDATES</span><h2>Release history</h2><p>Public changelog from <code>update.json</code>.</p></div><button class="btn" data-route="updates">${icon('history')} Changelog</button></div></section></div>`;
    bind(target, user, profile, toast); return true;
  }
