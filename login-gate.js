(() => {
  ...
  function ensureGate() {
    if (gate) return gate;
    gate = document.createElement('section'); gate.className = 'flash-login-gate'; gate.setAttribute('aria-live', 'polite'); document.body.appendChild(gate); return gate;
  }
  function lockBody() { document.body.classList.add('flash-auth-locked'); document.querySelectorAll('#app, #topbar, .bottom-dock, .side-panel, .modal-backdrop, .player-overlay').forEach((node) => { if (node !== gate) node.setAttribute('aria-hidden', 'true'); }); }
  function unlockBody() { document.body.classList.remove('flash-auth-locked'); document.querySelectorAll('#app, #topbar, .bottom-dock, .side-panel, .modal-backdrop, .player-overlay').forEach((node) => node.removeAttribute('aria-hidden')); }
  ...
  function allowOffline(user, cached) {
    if (!cached || cached.uid !== user.uid || cached.banStatus !== 'clear') return false;
    ensureGate().hidden = true; unlockBody(); return true;
  }
  async function evaluate(user) {
    if (!user) { renderLogin(); return; }
    const db = window.__flashFirebase?.db;
    if (navigator.onLine === false) {
      const cached = window.FlashOfflineSession?.read?.();
      if (allowOffline(user, cached)) return;
      setBlocked('restricted','You are offline and this account has not been verified on this device yet. Reconnect once to verify the account, then offline access will be available.'); return;
    }
    let profile = {};
    try { const snap = db ? await db.collection('users').doc(user.uid).get() : null; if (snap?.exists) profile = snap.data() || {}; }
    catch { const cached = window.FlashOfflineSession?.read?.(); if (allowOffline(user, cached)) return; setBlocked('restricted','Your account could not be verified right now.'); return; }
    if (profile.banned || profile.disabled || profile.deleted) { window.FlashOfflineSession?.clear?.(); setBlocked('banned','This account is not permitted to access Flash Games.'); return; }
    const until = Number(profile.suspendedUntil || 0);
    if (until > Date.now()) { window.FlashOfflineSession?.clear?.(); setBlocked('suspended',`This account is suspended until ${new Date(until).toLocaleString()}.`); return; }
    try { const global = await db.collection('settings').doc('unverifiedAccess').get(); if (profile.verified === false && global.exists && global.data()?.suspended === true) { setBlocked('unverified','Unverified accounts are temporarily restricted while verification is in progress.'); return; } } catch {}
    window.FlashOfflineSession?.write?.(user, profile); ensureGate().hidden = true; unlockBody();
  }
  function init() {
    const auth = window.__flashFirebase?.auth;
    if (!auth) { renderLogin('Firebase is unavailable.'); return; }
    if (authUnsubscribe) authUnsubscribe(); authUnsubscribe = auth.onAuthStateChanged(evaluate);
    window.addEventListener('online', () => { const current = auth.currentUser; if (current) evaluate(current); });
  }
