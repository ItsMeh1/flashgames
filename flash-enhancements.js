(() => {
  ...
  function enhanceOnlineDownloads() {
    ...
    const addButtons = () => document.querySelectorAll('.game-card[data-lumin-id]').forEach((card) => {
      ...
      button.addEventListener('click', async (event) => {
        ...
        try {
          const ok = await window.FlashOnlineCache?.cacheOne?.(games.get(rawId) || game);
  ...
  function terminalUI() {
    ...
    const colorize = (line) => {
      ...
      parts.forEach((part,index)=>{ if(!part)return; const span=document.createElement('span'); if(/^https?:\/\//i.test(part))span.className='syntax-url'; else if(/^\d/.test(part))span.className='syntax-number'; else if(/^\[.*\]$/.test(part))span.className='syntax-tag'; else if(/^(?:online|offline|controller|registration|state|connection|downlink|usage|quota|platform|cores|memory|language|fullscreen|supported|pictureInPicture|visibility|firebaseUser|uid|version)$/i.test(part))span.className='syntax-key'; else if(/^--/.test(part))span.className='syntax-flag'; else span.className=index%2?'syntax-text':''; span.textContent=part; frag.appendChild(span); });
      line.textContent=''; line.appendChild(frag); line.dataset.colored='1';
    };
    new MutationObserver(()=>output.querySelectorAll('.terminal-line').forEach(colorize)).observe(output,{childList:true,subtree:true});
    output.querySelectorAll('.terminal-line').forEach(colorize);
    if (!output.parentElement?.querySelector('.terminal-quick-actions')) {
      const quick=document.createElement('div'); quick.className='terminal-quick-actions'; quick.innerHTML=`<span>Quick actions</span><button type="button" data-command="help">Help</button><button type="button" data-command="system status">System</button><button type="button" data-command="browser viewport">Viewport</button><button type="button" data-command="modules install">Modules</button><button type="button" data-command="network status">Network</button>`; output.parentElement?.insertBefore(quick,output);
      quick.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{const input=document.getElementById('flashTerminalInput'),form=document.getElementById('flashTerminalForm');if(!input||!form)return;input.value=button.dataset.command||'';form.requestSubmit();}));
    }
  }
