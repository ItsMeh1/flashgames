(() => {
    ...
    const providers = [
      {
        ...
        description: 'Cloud gaming through Raccoon Game. Open the provider and choose a cloud title to start streaming.'
      },
      {
        id: 'yom-cloud-games',
        name: 'YOM Instant Cloud Games',
        url: 'https://yom.net/demo/',
        cover: './offline/logo.png',
        description: 'Browser-streamed cloud game demos with no local game download.'
      }
    ];
  ...
  function addPlayerLoader() {
    ...
    if (!overlay || !frame || overlay.dataset.phase3Loader) return !!overlay && !!frame;
    overlay.dataset.phase3Loader = '1';
    let loader = overlay.querySelector('[data-phase3-game-loader]');
    if (!loader) {
      ...
      loader.querySelector('strong').style.fontSize = '18px';
      loader.querySelector('span').style.opacity = '.68';
      const spinner = loader.querySelector('.phase3-game-spinner');
      Object.assign(spinner.style, { width: '28px', height: '28px', border: '2px solid rgba(255,255,255,.22)', borderTopColor: 'currentColor', borderRadius: '50%', animation: 'phase3Spin .8s linear infinite' });
      overlay.appendChild(loader);
      if (!document.getElementById('phase3-loader-style')) {
        const style = document.createElement('style');
        style.id = 'phase3-loader-style';
        style.textContent = '@keyframes phase3Spin{to{transform:rotate(360deg)}}';
    ...
    const observer = new MutationObserver(() => {
      if (!overlay.hidden && frame.src && frame.src !== 'about:blank') show();
      if (overlay.hidden) hide();
    });
    observer.observe(overlay, { attributes: true, attributeFilter: ['hidden'] });
    overlay.addEventListener('transitionend', () => { if (!overlay.hidden) show(); }, { passive: true });
  ...
  async function stabilizeDownloads() {
    ...
    const wrapped = async (game) => {
      const candidates = [...new Set([game?.rawUrl, game?.url, game?.href, game?.link].map((value) => String(value || '').trim()).filter((value) => /^https?:\/\//i.test(value)))];
      if (!candidates.length) return original(game);
      let lastError = null;
      for (const sourceUrl of candidates) {
        try {
          return await original({ ...game, rawUrl: sourceUrl, url: sourceUrl });
        } catch (error) {
          lastError = error;
          if (!/404|not found|download failed/i.test(String(error?.message || ''))) throw error;
        }
      }
      throw lastError || new Error('The game source could not be downloaded.');
    };
    wrapped.__phase3Wrapped = true;
    store.install = wrapped;
  }

  function boot() {
    installCloudProviders();
    addPlayerLoader();
    stabilizeDownloads();
    const playerObserver = new MutationObserver(() => addPlayerLoader());
